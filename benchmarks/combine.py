...
class Compare:
    def __init__(
        self, limit=None, datadir="~/data/combiner", dtype=None, with_uncertainty=True
    ):
        self.dtype = dtype
        ...
        if not datadir.exists():
            ...
            from ndcombine.helpers import make_fake_data
    ...
    def profile(self):
        ...
        profile.runcall(
            combine_arrays, self.ccds, method="mean", clipping_method="sigclip"
        )
        profile.print_stats()
    ...
    def measure_times(self, parallel=False, nrun=5, verbose=True, select_code=None):
        stats = []
...
if __name__ == "__main__":
    ...
    if args.mode == "line_profile":
        ...
        tbl = comp.measure_times(
            parallel=args.parallel, nrun=args.nrun, select_code=args.code
        )
        tbl.pprint(max_lines=-1, max_width=-1)
